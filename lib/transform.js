export const CATEGORIES = [
  ...
  { key: "reviews",       label: "Customer Reviews",    icon: "⭐", higherIsBetter: true,  source: "sheets" },
  ...
  { key: "p4pBonus",      label: "Bonus Payout ($)",    icon: "💵", higherIsBetter: true,  source: "sheets" },
  { key: "sameDayInvoiceRate", label: "Same-Day Invoice (%)", icon: "🧾", higherIsBetter: true, source: "hcp" },
];
...
export function mergeData(hcpTechs, sheetsData, slackData) {
  ...
  const hcpMap = {};
  (hcpTechs || []).forEach(t => { if (t.tech) hcpMap[t.tech.toLowerCase().trim()] = t; });

  function findHcp(name) {
    const key = name.toLowerCase().trim();
    if (hcpMap[key]) return hcpMap[key];
    for (const [k, v] of Object.entries(hcpMap)) {
      const kp = k.split(' '), np = key.split(' ');
      if (kp[0] === np[0] && kp[kp.length-1] === np[np.length-1]) return v;
    }
    return {};
  ...
  // Union of everyone who showed up in HCP job data AND everyone from the sheets
  // (sheets now includes the full active roster, not just techs with activity)
  const allNames = new Map();
  (hcpTechs || []).forEach(t => { if (t.tech) allNames.set(t.tech.toLowerCase().trim(), t.tech); });
  if (Array.isArray(sheetsData)) {
    sheetsData.forEach(row => { if (row.tech) allNames.set(row.tech.toLowerCase().trim(), row.tech); });
  }

  return [...allNames.values()].map(name => {
    const tech = findHcp(name);
    ...
    const sameDayInvoices = tech.sameDayInvoices ?? 0;
    const sameDayInvoiceRate = jobsCompleted > 0 ? Math.round((sameDayInvoices / jobsCompleted) * 100) : null;
    ...
    return {
      ...
      yardSigns,
      reviews: sheets.reviews ?? 0,
      tips: sheets.tips ?? 0,
      callbackRate,
      jobsCompleted,
      callbacks,
      callbacksByService: sheets.callbacksByService ?? {},
      sameDayInvoiceRate,
      sameDayInvoices,
...
export function getRankings(techs, key, higherIsBetter) {
  const needsJobs = key === 'callbackRate' || key === 'sameDayInvoiceRate';
  const eligible = needsJobs ? techs.filter(t => t.jobsCompleted > 0) : techs;
  const ineligible = needsJobs ? techs.filter(t => t.jobsCompleted === 0) : [];
...
export function computeOverall(techs) {
  ...
  CATEGORIES.forEach(cat => {
    const weight = CATEGORY_WEIGHTS[cat.key] ?? 1;
    const ranked = getRankings(active, cat.key, cat.higherIsBetter);
    // Only score if at least one tech has non-zero data in this category
    const hasAnyData = ranked.some(t => t[cat.key] !== null && t[cat.key] !== 0 && t[cat.key] !== undefined);
    if (!hasAnyData) return;
    ranked.filter(t => t[cat.key] !== null && t[cat.key] !== undefined)
      .forEach((t, i) => { scores[t.name] += (active.length - i) * weight; });
  });
  return [
