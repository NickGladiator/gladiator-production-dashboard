export const CATEGORIES = [
  ...
  { key: "missedSameDay", label: "Jobs Finished & Invoices Sent", icon: "🧾", higherIsBetter: false, source: "hcp" },
];
...
export function mergeData(hcpTechs, sheetsData, slackData) {
  ...
  return [...allNames.values()].map(name => {
    ...
    const missedSameDay = tech.missedSameDay ?? 0; // raw count — fewer is better, not a rate
...
export function getRankings(techs, key, higherIsBetter) {
  const needsJobs = key === 'callbackRate' || key === 'missedSameDay';
